...
export default function HomePage() {
    ...
    const initApp = async () => {
      try {
        ...
        if (liffModule.isLoggedIn()) {
          ...
          // ถ้าเปิดใน Chrome ทั่วไปหรือยังไม่ได้เข้าสู่ระบบ
  ...
  // Sync / Auto Register & Check Box Pairing Status
  const syncUserWithSupabase = async (lineUserId: string, name: string, pictureUrl?: string) => {
    try {
      ...
      if (!user) {
        // สร้าง User ใหม่หากยังไม่มีในระบบ
      ...
      if (user) {
        // ตรวจสอบว่าผู้ใช้ผูกกล่องยาแล้วหรือยัง (หากไม่มี device_id หรือกล่องยา ให้ไป Onboarding)
        if (!user.device_id) {
    ...
    <div className={`min-h-screen bg-slate-50 text-slate-800 ${fontSizeClass} pb-12`}>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded-3xl max-w-md w-full space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleSaveMedicine} className="space-y-4">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">ชื่อยา</label>
                <input 
                  type="text" 
                  required 
                  value={medForm.name} 
                  onChange={(e) => setMedForm({ ...medForm, name: e.target.value })} 
                  className="w-full p-3 rounded-2xl border border-slate-300 font-medium" 
                  placeholder="เช่น ยาความดัน"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">จำนวนทั้งหมด (เม็ด)</label>
                  <input 
                    type="number" 
                    required 
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">คงเหลือ (เม็ด)</label>
                  <input 
                    type="number" 
                    required 
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">วันหมดอายุ (ถ้ามี)</label>
                <input 
                  type="date" 
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={closeMedModal} className="px-5 py-2.5 rounded-2xl border border-slate-300 font-bold text-slate-600">ยกเลิก</button>
                <button type="submit" className="px-5 py-2.5 rounded-2xl bg-emerald-600 text-white font-bold">บันทึก</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white p-6 rounded-3xl max-w-md w-full space-y-4 shadow-2xl">
            ...
            <form onSubmit={handleSaveSchedule} className="space-y-4">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">เลือกรายการยา</label>
                <select 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">เวลาเตือน</label>
                  <input 
                    type="time" 
                    required 
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-1">จำนวนทาน (เม็ด)</label>
                  <input 
                    type="number" 
                    required 
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="activeCheck" className="text-sm font-bold text-slate-700">เปิดใช้งานการแจ้งเตือนนี้</label>
              </div>
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={closeScheduleModal} className="px-5 py-2.5 rounded-2xl border border-slate-300 font-bold text-slate-600">ยกเลิก</button>
                <button type="submit" className="px-5 py-2.5 rounded-2xl bg-emerald-600 text-white font-bold">บันทึก</button>
              </div>
            </form>
          </div>
      ...
      {selectedImage && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelectedImage(null)}>
          <div className="relative max-w-lg w-full bg-white rounded-3xl p-3 overflow-hidden shadow-2xl">
            <button onClick={() => setSelectedImage(null)} className="absolute top-5 right-5 bg-black/50 text-white p-2 rounded-full">
              <X className="w-6 h-6" />
            </button>
            <img src={selectedImage} alt="หลักฐานการกินยา" className="w-full h-auto rounded-2xl max-h-[80vh] object-contain" />
          </div>
        </div>
      )}
    </div>
